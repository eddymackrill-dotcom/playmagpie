...
// Per-coin strip configuration. Top 3 of casinos accepting that coin
...
const intros: Record<string, string> = {
  ...
  USDT: 'USDT (Tether) is the most practical gambling cryptocurrency available in 2026, particularly on the TRC-20 (Tron) network. As a stablecoin pegged 1:1 to the US dollar, USDT eliminates the bankroll volatility that comes with BTC or ETH. A $500 USDT deposit is worth $500 when you withdraw, regardless of what crypto markets do between sessions. On TRC-20, confirmations complete in under three seconds with fees consistently below $0.01. Every major crypto casino accepts USDT, often across multiple networks (TRC-20, ERC-20, and increasingly Solana). The same logic applies to USD Coin (USDC), the second-largest stablecoin: issued by Circle and audited monthly, it offers identical dollar-pegged stability with stronger regulatory positioning and monthly reserve attestations, and confirms in under a second on Solana with negligible fees. BC.Game, Cloudbet and Shuffle all support USDC deposits and withdrawals, so players with concerns about Tether reserves can run the identical playbook with USDC and give up little in speed or acceptance. For players who want fast, cheap, predictable transactions and a fixed-dollar bankroll, USDT on TRC-20 is the recommended default, with USDC the natural alternative.',
  ...
  BNB: 'BNB on BNB Smart Chain settles in roughly three seconds at sub-cent fees, among the cheapest end-to-end deposit and withdrawal paths at any crypto casino. Casino acceptance is narrower than for BTC, ETH or USDT: six of the eight casinos in our rankings accept BNB directly (BC.Game, Cloudbet, 7Bit Casino, Shuffle, Duelbits and, since 2026, Roobet), while BitStarz and Mirax Casino do not. BNB is non-stablecoin: its dollar value moves with market sentiment, so balances held on platform across weeks drift relative to USD, unlike USDT or USDC. The practical use case is depositing what you already hold (typically from Binance exchange usage or BNB Chain DeFi positions) to avoid conversion overhead; there is no scenario where you would specifically buy BNB to gamble with rather than buying USDT or SOL. BNB moves on BNB Smart Chain (BEP-20); the legacy Beacon Chain was retired in November 2024, so BEP-20 is the only BNB network a casino cashier will offer. Confirm the receiving address is BEP-20 before withdrawing from an exchange.',
}
