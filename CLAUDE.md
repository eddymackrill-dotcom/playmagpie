...
**Permanently unfetchable sources.** Where a primary cannot be fetched for structural reasons, mark it ONCE in the catalogue as permanently unfetchable with the reason. After that it is a known condition, not a finding, and is not re-raised on each figure that depends on it. Standing entry: Roobet terms and conditions, JavaScript shell, does not render to a fetch. Catalogue values from the owner's May 2026 verbatim read stand for stable facts. Permanent condition, settled, do not re-raise. Second standing entry (owner decision 2026-08-25): per-page DAILY AI-citation series, Bing WMT AI Performance. The UI exports site-wide dailies and per-page window totals only; per-page dailies are structurally unavailable and their absence is never re-raised per pull. Checkpoint A's per-page basis is DIFFERENCED CONSECUTIVE PULLS: difference each page's cumulative total between successive exports and divide by the days between them (series points 17 Aug, 22 Aug, 25 Aug 2026 onward).
