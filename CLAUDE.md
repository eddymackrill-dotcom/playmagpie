...
**Crawl-discovery rule (hard rule, 2026-06-04):** every new page in a low-crawl section (`/bonus/*`, `/crypto/*`, `/game/*` leaves, and similar intent-page subdirectories) must ship with **≥1 inbound contextual prose link from a same-batch-modified, frequently-crawled page** (typically the `/reviews/*` tree) to guarantee *timely* discovery. Hub auto-tiles (e.g. the `/bonus` hub mapping over `BONUS_TYPES`) and sitemap inclusion do enable *eventual* discovery, but both depend on Google re-crawling the hub/sitemap on its own schedule — which can leave a new page stranded in "URL is unknown to Google" for weeks. Evidence: `/bonus/free-spins` sat uncrawled for ~2 weeks despite being live, in the sitemap, and carrying a hub tile + one prose link, because neither linking page had been re-crawled since the link was added; meanwhile same-batch pages on the actively-crawled `/reviews/*` tree indexed within a day. The fix is structural, not a one-off: pushing a contextual link from a page that *will* be re-crawled (because it was modified in the same batch) pulls the new page forward in the crawl queue.
